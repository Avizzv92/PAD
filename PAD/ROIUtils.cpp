...
void ROIUtils::addLabel(cv::Mat& image, const ROI &roi) {
    ...
    rectangle(image, point + Point(-textSize.width/2, baseline), point + Point(textSize.width/2, -textSize.height), CV_RGB(0,0,0), CV_FILLED);
    putText(image, label, point + Point(-textSize.width/2,3), FONT_HERSHEY_PLAIN, 1.0, CV_RGB(255,255,255), 1, 8);
}
